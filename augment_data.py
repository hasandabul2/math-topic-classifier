...
DEFAULT_AUGMENTATION_FRACTIONS = {
    0: (0.0, 0.0),  # Algebra          (2618 samples - already large, no aug needed)
    1: (0.0, 0.0),  # Geometry & Trig  (2439 samples - already large)
    2: (1.0, 0.5),  # Calculus         (1039 → ~2600 with aug)
    3: (5.0, 2.0),  # Prob & Stats     ( 368 → ~2900 with aug)
    4: (0.3, 0.2),  # Number Theory    (1712 → ~2600 with aug)
    5: (0.2, 0.2),  # Combinatorics    (1827 → ~2500 with aug)
    6: (15.0, 10.0), # Linear Algebra  ( 100 → ~2600 with aug)
    7: (18.0, 12.0)  # Abstract Alg    (  86 → ~2600 with aug)
}
...
def load_translation_models(translation_model, back_translation_model, device):
    ...
    # Import transformers lazily to avoid top-level dependency for environments without it
    import importlib
    transformers = importlib.import_module("transformers")

    # Forward translation model (e.g., en -> de)
    f_tok = transformers.AutoTokenizer.from_pretrained(translation_model)
    f_mod = transformers.AutoModelForSeq2SeqLM.from_pretrained(translation_model).to(device)
    # Backward translation model (e.g., de -> en)
    b_tok = transformers.AutoTokenizer.from_pretrained(back_translation_model)
    b_mod = transformers.AutoModelForSeq2SeqLM.from_pretrained(back_translation_model).to(device)
...
def back_translate(questions, forward_pair, backward_pair, device, batch_size=8):
    ...
    try:
        import importlib
        torch = importlib.import_module("torch")  # Avoid static import to appease linters without torch installed
    except Exception as exc:
        raise RuntimeError("torch is required for back-translation. Please install PyTorch or disable translation augmentation.") from exc
...
def main():
    ...
    if 'id' not in df.columns:
        df['id'] = [f'q_{i}' for i in range(len(df))]
    ...
    if translation_needed:
        # Resolve device without requiring torch at import time
        ...
        if args.device.lower() == "cuda":
            try:
                ...
                print("PyTorch not installed. Falling back to CPU for translation.")
                resolved_device = "cpu"
        ...
        if trans_frac > 0:
            ...
            if trans_count > 0:
                ...
                translated, valid_indices = back_translate(
                    ...
                    forward_pair,
                    backward_pair,
                    resolved_device if 'resolved_device' in locals() else args.device,
                    args.batch_size
                )
